...
func applyStudentIdentity(s string, opts Options) string {
	...
	out = strings.ReplaceAll(out, "[isi nama]", opts.StudentName)
	out = strings.ReplaceAll(out, "[Isikan Nama Lengkap]", opts.StudentName)
	out = strings.ReplaceAll(out, "[isikan nama lengkap]", opts.StudentName)
	out = strings.ReplaceAll(out, "[Isi NIM]", opts.StudentNIM)
	out = strings.ReplaceAll(out, "[isi nim]", opts.StudentNIM)
	out = strings.ReplaceAll(out, "[Isikan NIM]", opts.StudentNIM)
	out = strings.ReplaceAll(out, "[isikan nim]", opts.StudentNIM)
	out = regexp.MustCompile(`(?i)\[(?:isi|isikan)\s+nama(?:\s+lengkap)?\]`).ReplaceAllString(out, opts.StudentName)
	out = regexp.MustCompile(`(?i)\[(?:isi|isikan)\s+nim\]`).ReplaceAllString(out, opts.StudentNIM)
