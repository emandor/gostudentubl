...
func main() {
	...
	r := &runner.Runner{
		...
		RunLockEnabled:                cfg.RunLockEnabled,
		RunLockTTL:                    time.Duration(cfg.RunLockTTLMinutes) * time.Minute,
		MaterialSyncEnabled:           cfg.MaterialSyncEnabled,
		MaterialSyncLimit:             cfg.MaterialSyncLimit,
		MaterialCacheDir:              cfg.MaterialCacheDir,
		MaterialMaxFileMB:             cfg.MaterialMaxFileMB,
		PDFGenerationEnabled:          cfg.PDFGenerationEnabled,
		PDFCacheDir:                   cfg.PDFCacheDir,
		PDFStudentName:                cfg.PDFStudentName,
		PDFStudentNIM:                 cfg.PDFStudentNIM,
		ChromiumPath:                  cfg.ChromiumPath,
		SubmissionEnabled:             cfg.SubmissionEnabled,
		SubmissionDeadlineWindowHours: cfg.SubmissionDeadlineWindowHours,
	}
	...
	if cfg.DailySummaryEnabled {
		if err := jobs.AddDailySummary(cfg.CronDailySummary, r); err != nil {
			log.Fatal().Err(err).Msg("adding daily summary job")
		}
	}
